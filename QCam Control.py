...
class Camera:     
    ...
    def grab_frame(self):
        ...
        if result == 0:  # replace with your actual success code
            return frame
        else:
...
if __name__ == "__main__":
    ...
    try:
        # =============================================================================○
        # Code to Initalize Camera
        # =============================================================================
        # Create a camera object
        camera = Camera()
    
        # Load the driver
        result = camera.load_driver()
        if result != 0:  # Replace with your actual success value
        ...
        # Call list_cameras
        result = camera.list_cameras(ctypes.byref(pList[0]), ctypes.byref(pNumberInList))
        ...
        result = camera.open_camera(pList[0].cameraId)
        if result != 0:  # Replace with your actual success value
            print(f"Camera opening failed with error {result}")
            sys.exit(1)
        
        print(f"Camera opened with handle {camera.camera_handle.value}")
        cameraOpened = True
